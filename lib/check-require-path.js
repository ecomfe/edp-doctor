    ...
    list.forEach(function (item) {
        ...
        if (requireMods && requireMods.length > 0) {
            requireMods.forEach(function (mod) {
                callback(mod, fileName, filePath);
            });
        }
    });
}
...
module.exports = function(args, opts) {
    ...
    // 检测project类项目依赖于module.conf
        ...
        analyzeModPath(fileList, function (mod, fileName, filePath) {
            ...
            if (PREFIX_DOT.test(mod)) {
                // 当前模块的top-level id
                var topLevelId = edp.esl.getModuleId(filePath, moduleConfigPath);

                // 以当前模块的top-level id作为baseId去将依赖的相对的模块id
                // 转换为绝对id
                // 当前依赖模块的top-level id
                var requireModTopLevelId = edp.esl.resolveModuleId(
                    mod,
                    topLevelId[0]
                );

                modPath = edp.esl.getModuleFile(
                    requireModTopLevelId,
                    moduleConfigPath
                );
            }
            else {
                modPath = edp.esl.getModuleFile(mod, moduleConfigPath);
            }

            if (isPlugin) {
                modPath = modPath.replace(/\.js$/, '');
            }
