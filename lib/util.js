/**
 * @file lib/util.js ~ 2014/03/29 16:12:13
...
exports.checkRequirePath = function (fileset) {
    ...
    function addInvalidResource(resource, file) {
        var ignored = false;
        ignoreModules.some(function (item) {
            if (edp.path.satisfy(resource, item)) {
                ignored = true;
                return true;
            }
        });
    ...
    if (fs.existsSync(packageJSON)) {
        ...
        if (packageConfig.edp && packageConfig.edp.doctor) {
            ignoreModules = packageConfig.edp.doctor.ignoreModules || [];
        }
    }
        ...
        mod.actualDependencies.forEach(function (depId) {
            ...
            if (rv.resource) {
                ...
                if (!fs.existsSync(resourceUrl)) {
                    addInvalidResource(depId, mod.file);
                }
            }
    ...
    function dump(resourceUrl) {
        edp.log.error('→ No such module id: %s', resourceUrl);
    }
