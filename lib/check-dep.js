 ...
 * @param {string} item dep下的子目录/子文件
 */
function checkDepV2(item) {
    var invalidResource = [];

    item = edp.path.join('dep', item);

    var stat = fs.statSync(item);
    if (stat.isFile() && !/\.md5$/.test(item)) {
        invalidResource.push({
            type: 1,
            path: item
        });
        ...
        var pkgFile = edp.path.join(item, 'package.json');
...
module.exports = exports = function (args, opts) {
    ...
    if (fs.existsSync('dep')) {
        if (fs.existsSync('package.json')) {
            ...
            if (pkg && pkg.edp && pkg.edp.layout === 'v2') {
                fs.readdirSync('dep').forEach(checkDepV2);
                return;
            }
        }
